...
async function updateScholarshipRating(scholarshipId) {
    ...
    scholarship.averageRating = reviews.length > 0 ? (totalRating / reviews.length).toFixed(2) : 0;
...
exports.getAllReviews = async (req, res) => {
    try {
        const reviews = await Review.find({}).populate('scholarshipId', 'scholarshipName universityName');
        res.json(reviews);
    } catch (err) {
...
exports.updateMyReview = async (req, res) => {
    try {
        ...
        if (!review) return res.status(404).json({ message: 'Review not found or you are not authorized to edit it.' });
...
exports.deleteMyReview = async (req, res) => {
    try {
        ...
        if (!review) return res.status(404).json({ message: 'Review not found or you are not authorized to delete it.' });

        await Scholarship.updateOne({ _id: review.scholarshipId }, { $pull: { reviews: review._id } });
        await updateScholarshipRating(review.scholarshipId);
